...
export const $fc = (option?: FrourioClientOption) => ({
  ...
  async $post(req: Parameters<ReturnType<typeof methods_11ohl7p>['$post']>[0]): Promise<z.infer<typeof frourioSpec_11ohl7p.post.res[200]['body']>> {
...
const $url_11ohl7p = (option?: FrourioClientOption) => ({
  post(): { isValid: true; data: string; reason?: undefined } | { isValid: false, data?: undefined; reason: z.ZodError } {
    return { isValid: true, data: `${option?.baseURL?.replace(/\/$/, '') ?? ''}D:/Data/Develop/software/moe-equipment-simulator/src/app/api/search/result` };
  },
});

const methods_11ohl7p = (option?: FrourioClientOption) => ({
  async $post(req: { body: z.infer<typeof frourioSpec_11ohl7p.post.body>, init?: RequestInit }): Promise<
    | { ok: true; isValid: true; data: { status: 200; headers?: undefined; body: z.infer<typeof frourioSpec_11ohl7p.post.res[200]['body']> }; failure?: undefined; raw: Response; reason?: undefined; error?: undefined }
  ...
  > {
    ...
    switch (result.res.status) {
      case 200: {
        ...
        const body = frourioSpec_11ohl7p.post.res[200].body.safeParse(resBody.data);
