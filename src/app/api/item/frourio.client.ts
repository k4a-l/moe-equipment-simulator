...
export const $fc = (option?: FrourioClientOption) => ({
	...
	async $post(
		req: Parameters<ReturnType<typeof methods_10pvn8k>["$post"]>[0],
	): Promise<z.infer<(typeof frourioSpec_10pvn8k.post.res)[200]["body"]>> {
...
const $url_10pvn8k = (option?: FrourioClientOption) => ({
	post():
		...
		return {
			isValid: true,
			data: `${option?.baseURL?.replace(/\/$/, "") ?? ""}D:/Data/Develop/software/moe-equipment-simulator/src/app/api/item`,
		};
	},
});
...
const methods_10pvn8k = (option?: FrourioClientOption) => ({
	async $post(req: {
		body: z.infer<typeof frourioSpec_10pvn8k.post.body>;
		init?: RequestInit;
	}): Promise<
		| {
				ok: true;
				...
				data: {
					status: 200;
					headers?: undefined;
					body: z.infer<(typeof frourioSpec_10pvn8k.post.res)[200]["body"]>;
				};
				failure?: undefined;
				raw: Response;
				reason?: undefined;
	...
	> {
		...
		switch (result.res.status) {
			case 200: {
				...
				const body = frourioSpec_10pvn8k.post.res[200].body.safeParse(
					resBody.data,
				);
